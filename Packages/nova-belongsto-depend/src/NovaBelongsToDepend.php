...
class NovaBelongsToDepend extends BelongsTo
{
    ...
    public function forResource(string $resource, ?int $resource_id = null, ?string $relation = null): self
    {
        $resource = class_exists($resource) ? $resource::uriKey() : null;
        $this->withMeta([
                            'viaResource' => $resource,
                            'viaResourceId' => $resource_id,
                            'viaRelationship' => $relation,
                        ]);
    ...
    public function sortableUriKey()
    {
        ...
        try {
            ...
            return parent::sortableUriKey();
        }

        return $this->getRelationForeignKeyName($resource->resource->{$this->attribute}());
    }
