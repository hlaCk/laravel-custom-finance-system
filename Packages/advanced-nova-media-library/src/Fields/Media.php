...
class Media extends Field
{
    ...
    protected function handleMedia(NovaRequest $request, $model, $attribute, $data)
    {
        ...
        if ($model->cloneMedia) {
            $this->addNewMediaWithClone($request, $model, $attribute, $data);
            $existingIds = $this->addExistingMediaWithClone($request, $data, $model, $attribute, $model->getMedia($attribute));
            // As there going to be one file attached with the model so no need of setOrder()
        } else {
    ...
    public function addNewMediaWithClone(NovaRequest $request, $model, $attribute, $data)
    {
        $collection = $attribute;
        // get total no of files
        $uploadedFiles = collect($data)
            ->filter(function($value) {
                ...
                // Now each time we need to create a new $model instance and then need to attach single media file to it.
                if ($index == 0) {
                    $modelClone = $model;
                } else {
                ...
                if( $file instanceof UploadedFile ) {
                    $media = $modelClone->addMedia($file)->withCustomProperties($this->customProperties);
                } else {
                    $media = $this->makeMediaFromVaporUpload($file, $modelClone);
                }
                ...
                if( is_callable($this->setFileNameCallback) ) {
                    $media->setFileName(
                        call_user_func($this->setFileNameCallback, $file->getClientOriginalName(), $file->getClientOriginalExtension(), $modelClone)
                    );
                }

                if( is_callable($this->setNameCallback) ) {
                    $media->setName(
                        call_user_func($this->setNameCallback, $file->getClientOriginalName(), $modelClone)
                    );
                }
