...
class NovaBreadcrumbsController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if ($this->resource && (Str::lower(Str::title($view ))== 'index' || Str::lower(Str::title($view )) == 'create' || Str::lower(Str::title($view )) == 'edit' || Str::lower(Str::title($view )) == 'detail')) {
            if (method_exists($this->resource, 'breadcrumbResourceTitle')) {
                $customBreadCrumb = $this->resource::customBreadcrumb();
                if (count($customBreadCrumb) > 0 && isset($customBreadCrumb[Str::lower(Str::title($view))])) {
                    $this->showCustomBreadCrumb(Str::lower(Str::title($view)), $pathParts);
    ...
    public function showCustomBreadCrumb($view, $pathParts)
    {
        $this->crumbs = new Collection();
        $customBreadCrumbs = $this->resource::customBreadcrumb();
        foreach($customBreadCrumbs[$view] as $customBreadCrumb) {
            $this->appendToCrumbs($customBreadCrumb['title'], $customBreadCrumb['url']);
        }

        if ($pathParts->has(2) && ($view == 'edit' || $view == 'detail')) {
            $this->resource = Nova::resourceForKey($pathParts->get(1));
            $this->model = $this->findResourceOrFail($pathParts->get(2));
            if (method_exists($this->model, 'breadcrumbResourceTitle')) {
                $this->appendToCrumbs($this->model->breadcrumbResourceTitle(),
