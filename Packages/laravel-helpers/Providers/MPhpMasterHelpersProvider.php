...
class MPhpMasterHelpersProvider extends ServiceProvider
{
    ...
    public function registerMacros()
    {
        ...
        $namespace = basename($namespace) === "Providers" ? dirname($namespace) : $namespace;
...
//            glob(real_path($cutBasePath(self::MIXINS_DIR__ . "/*Invoke.php")))
            glob(fixPath(real_path($cutBasePath("/*Invoke.php", self::MIXINS_DIR__))))
                  ...
                  ->each(static function ($macro, $path) use ($cutBasePath) {
                      $class = str_ireplace('/', NAMESPACE_SEPARATOR, $cutBasePath($path));
    ...
    public function boot(Router $router)
    {
        ...
        if( config('app.debug') && (env('CUSTOM_FUNCTIONS') ?? true) ) {
            new \CustomFunctions;
        }
    }
