...
if (!function_exists('nova_get_menu_by_slug')) {
    function nova_get_menu_by_slug($slug, $locale = null, ?Closure $callback = null)
    {
        $callback = $callback ?? fn($_menu) => $_menu->formatForAPI($locale);
        if (empty($slug)) return null;

        if (empty($locale)) $locale = array_keys(MenuBuilder::getLocales())[0] ?? null;

        $menu = MenuBuilder::getMenuClass()::where('slug', $slug)->first();
        return !empty($menu) ? $callback($menu,$locale) : null;
    }
}
